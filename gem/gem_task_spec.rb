...
describe New::GemTask do
  before do
    @gem = New::GemTask.new({:test_files => 'spec'})
  end

  it 'should build glob attributes' do
    expect(@gem.instance_var(:gemspec)[:files]).to eq 'asd'
    expect(@gem.instance_var(:gemspec)[:test_files]).to eq 'asd'
  end
  # before do
  #   stub_const 'New::CONFIG_FILE', New::CONFIG_FILE
  #   stub_const 'New::GemTask::GLOB_ATTRIBUTES', [:foo_files]
  ...
  #   before do
  #     stub_const 'New::GemTask::GEMFILE', root('spec', 'fixtures', 'tasks', 'foo_task', 'Gemfile')
  #   end
